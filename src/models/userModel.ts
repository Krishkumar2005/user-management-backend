...
const runAsync = (sql: string, params: any[] = []) =>
  new Promise<{ lastID: number }>((resolve, reject) => {
...
export const getAllUsers = async (
  ...
  return (await allAsync(query, params)) as User[];
};


// ✅ GET BY ID
export const getUserById = async (id: number): Promise<User | undefined> => {
  return (await getAsync("SELECT * FROM users WHERE id = ?", [id])) as User;
};


// ✅ CREATE
export const createUser = async ({ name, email, age }: UserInput): Promise<User> => {
